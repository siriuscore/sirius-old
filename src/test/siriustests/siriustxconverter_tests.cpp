...
void checkResult(bool isCreation, std::vector<SiriusTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    SiriusTxConverter converter(transaction, NULL);
    ExtractSiriusTX siriusTx;
    BOOST_CHECK(converter.extractionSiriusTransactions(siriusTx));
    std::vector<SiriusTransaction> result = siriusTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(siriustxconverter_tests, TestingSetup)
