...
SiriusState::SiriusState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = SiriusState::openDB(_path + "/siriusDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

SiriusState::SiriusState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute SiriusState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, SiriusTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createSiriusAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            sirius::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> SiriusState::vins() const // temp
{
...
void SiriusState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* SiriusState::vin(dev::Address const& _a) const
{
    return const_cast<SiriusState*>(this)->vin(_a);
}

Vin* SiriusState::vin(dev::Address const& _addr)
{
...
// void SiriusState::commit(CommitBehaviour _commitBehaviour)
// {
...
void SiriusState::kill(dev::Address _addr)
{
...
void SiriusState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void SiriusState::deleteAccounts(std::set<dev::Address>& addrs){
...
void SiriusState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void SiriusState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
