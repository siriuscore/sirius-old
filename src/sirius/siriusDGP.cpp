...
bool SiriusDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule SiriusDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t SiriusDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t SiriusDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t SiriusDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t SiriusDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool SiriusDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void SiriusDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void SiriusDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void SiriusDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void SiriusDGP::createParamsInstance(){
...
dev::Address SiriusDGP::getAddressForBlock(unsigned int blockHeight){
...
void SiriusDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SiriusDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SiriusDGP::parseStorageOneUint64(uint64_t& value){
...
void SiriusDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule SiriusDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
