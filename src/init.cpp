    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                boost::filesystem::path siriusStateDir = GetDataDir() / "stateSirius";
                bool fStatus = boost::filesystem::exists(siriusStateDir);
                const std::string dirSirius(siriusStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsSIRXtate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<SiriusState>(new SiriusState(dev::u256(0), SiriusState::openDB(dirSirius, hashDB, dev::WithExisting::Trust), dirSirius, existsSIRXtate));
