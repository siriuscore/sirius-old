...
#define paternVersion "sirius-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

SiriusVersionChecker::SiriusVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

SiriusVersionChecker::~SiriusVersionChecker()
{

}

bool SiriusVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> SiriusVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(SIRX_RELEASES)));
    QEventLoop event;
...
Version SiriusVersionChecker::getMaxReleaseVersion()
{
