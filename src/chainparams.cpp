...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
{
    ...
    genesis.hashStateRoot = uint256(h256Touint(dev::h256("e965ffd002cd6ad0e2dc402b8044de833e06b23127ea8c3d80aec91410771495"))); // sirius
    genesis.hashUTXORoot = uint256(h256Touint(dev::sha3(dev::rlp("")))); // sirius
    return genesis;
}
...
public:
    CMainParams() {
        ...
        vSeeds.emplace_back("46.101.164.93", false); // Sirius mainnet
        vSeeds.emplace_back("159.89.47.201", false); // Sirius mainnet
        vSeeds.emplace_back("163.172.71.184", false); // Sirius mainnet
        vSeeds.emplace_back("173.82.15.7", false); // Sirius mainnet
	vSeeds.emplace_back("94.130.15.169", false); // Sirius mainnet
	vSeeds.emplace_back("104.225.219.29", false); // Sirius mainnet
	vSeeds.emplace_back("93.104.209.17", false); // Sirius mainnet
	vSeeds.emplace_back("104.251.219.220", false); // Sirius mainnet
...
public:
    CTestNetParams() {
        ...
        // The best chain should have at least this much work.
        consensus.nMinimumChainWork = uint256S("0x00000000000000000000000000000000000000000000000d6e528bcf5a3a556f"); // sirius
...
public:
    CRegTestParams() {
        ...
        consensus.BIP34Height = 0; // BIP34 has not activated on regtest (far in the future so block v1 are not rejected in tests) // activate for sirius
...
public:
    CUnitTestParams()
    {
        ...
        // SIRX have 500 blocks of maturity, increased values for regtest in unit tests in order to correspond with it
