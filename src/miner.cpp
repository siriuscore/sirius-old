...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setSiriusSchedule(siriusDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = siriusDGP.getBlockSize(nHeight);
    minGasPrice = siriusDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = siriusDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    SiriusTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractSiriusTX resultConverter;
    if(!convert.extractionSiriusTransactions(resultConverter)){
    ...
    for(SiriusTransaction siriusTransaction : siriusTransactions){
        txGas += siriusTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + siriusTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(siriusTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, siriusTransactions, hardBlockGasLimit);
...
void BlockAssembler::addPackageTxs(int &nPackagesSelected, int &nDescendantsUpdated, uint64_t minGasPrice)
{
    ...
    while (mi != mempool.mapTx.get<ancestor_score_or_gas_price>().end() || !mapModifiedTx.empty())
    {
        ...
        bool wasAdded=true; // SIRX_INSERT_LINE
...
void StakeSiriuss(bool fStake, CWallet *pwallet)
{
