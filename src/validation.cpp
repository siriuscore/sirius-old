...
bool fReindex = false;
bool fTxIndex = false;
bool fAddressIndex = false; // sirius
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            for(SiriusTransaction siriusTransaction : siriusTransactions){
                ...
                if(siriusTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SIRX additional fees, %d delta bytes\n",
