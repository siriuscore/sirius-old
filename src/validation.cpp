...
#if defined(NDEBUG)
# error "Sirius cannot be compiled without assertions."
#endif
...
std::unique_ptr<SiriusState> globalState;
...
const std::string strMessageMagic = "Sirius Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // sirius
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = siriusDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            ...
            SiriusTxConverter converter(tx, NULL);
            ExtractSiriusTX resultConverter;
            if(!converter.extractionSiriusTransactions(resultConverter)){
            ...
            std::vector<SiriusTransaction> siriusTransactions = resultConverter.first;
            std::vector<EthTransactionParams> siriusETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(SiriusTransaction siriusTransaction : siriusTransactions){
                sumGas += siriusTransaction.gas() * siriusTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, siriusTransaction.gasPrice());
                } else {
                    txMinGasPrice = siriusTransaction.gasPrice();
                }
                VersionVM v = siriusTransaction.getVersion();
                if(v.format!=0)
                ...
                if(siriusTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(siriusTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += siriusTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)siriusTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(siriusETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > siriusTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s SIRX additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // sirius
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // sirius

    if(pfClean == NULL && fLogEvents){
        boost::filesystem::path stateDir = GetDataDir() / "stateSirius";
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    SiriusTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<SiriusTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path siriusDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(siriusDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(siriusDir.string(), std::ios::in | std::ios::out);
...
bool SiriusTxConverter::extractionSiriusTransactions(ExtractSiriusTX& siriustx){
    std::vector<SiriusTransaction> resultTX;
    ...
    siriustx = std::make_pair(resultTX, resultETP);
    return true;
}

bool SiriusTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool SiriusTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
SiriusTransaction SiriusTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    SiriusTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = SiriusTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = SiriusTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // sirius
    SiriusDGP siriusDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setSiriusSchedule(siriusDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = siriusDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = siriusDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = siriusDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // sirius
...
///////////////////////////////////////////////////////////////////////////////////////// sirius
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            SiriusTxConverter convert(tx, &view, &block.vtx);

            ExtractSiriusTX resultConvertSiriusTX;
            if(!convert.extractionSiriusTransactions(resultConvertSiriusTX)){
            ...
            if(!CheckMinGasPrice(resultConvertSiriusTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertSiriusTX.first, blockGasLimit);
            ...
            for(SiriusTransaction& qtx : resultConvertSiriusTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertSiriusTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertSiriusTX.first[k].from(), resultConvertSiriusTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // sirius
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // sirius
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // sirius
            globalState->setRootUTXO(oldHashUTXORoot); // sirius
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // sirius
        uint32_t sizeBlockDGP = siriusDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Sirius PoS we need to sync this after every block to ensure txdb is populated for
