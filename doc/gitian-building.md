...
*Setup instructions for a Gitian build of Sirius Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sirius
...
- [Building Sirius Core](#building-sirius-core)
...
Clone the git repositories for sirius and Gitian.
...
```bash
sudo cp /home/debian/sirius/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the sirius repository under 'Fetch and create inputs' to install sources which require
...
To build Sirius Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sirius repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sirius/.git/
...
For example:
```bash
URL=https://github.com/laanwj/sirius.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sirius=${COMMIT} --url sirius=${URL} ../sirius/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sirius=${COMMIT} --url sirius=${URL} ../sirius/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sirius=${COMMIT} --url sirius=${URL} ../sirius/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-sirius:
./bin/gbuild --commit sirius=${COMMIT},cpp-eth-sirius=${ETHCOMMIT} --url sirius=${URL},cpp-eth-sirius=${ETHURL} ../sirius/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the sirius git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sirius/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sirius
SIGPATH=/some/root/path/sirius-detached-sigs

./bin/gbuild --url sirius=${BTCPATH},signature=${SIGPATH} ../sirius/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sirius-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sirius-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sirius-osx-build.assert
```
...
[siriusproject/gitian.sigs](https://github.com/siriusproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@sirius.org) and he will commit them.
